...
export const BadgeCollection: React.FC<BadgeCollectionProps> = ({ unlockedBadges, onBadgeCreated }) => {
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      {/* Header & Actions */}
      <div className="flex justify-between items-center bg-white p-6 rounded-xl shadow-sm border border-ocean-100">
        <div>
           <h2 className="text-xl font-bold text-gray-900 flex items-center">
             <span className="mr-2">🐋</span> 생물 배지
           </h2>
           <p className="text-sm text-gray-500 mt-1">직접 촬영한 해양 생물 사진으로 나만의 특별한 배지 컬렉션을 완성해보세요.</p>
        </div>
        <Button 
          onClick={() => setIsMakerOpen(true)}
          className="bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 border-none shadow-md whitespace-nowrap ml-4"
          icon={<span>✨</span>}
        >
          생물 배지 만들기
        </Button>
